class BioVAuth {
    constructor() {
        ...
        this.elements = {
            ...
            fallbackBtn: document.getElementById('fallbackBtn'), visitBtn: document.getElementById('visitBtn'),
            webauthnBtn: document.getElementById('webauthnBtn'), emailInput: document.getElementById('email'),
            passwordInput: document.getElementById('password'), submitLogin: document.getElementById('submitLogin'),
            loadingSpinner: document.getElementById('loadingSpinner'), errorMsg: document.getElementById('errorMsg'),
    ...
    async collectClientInfo() {
        try {
            ...
            console.error('Error collecting client info:', error); // Dibiarkan untuk debugging
            this.state.clientInfo = {
                ip: 'unavailable', userAgent: navigator.userAgent,
                platform: navigator.platform || 'unknown', timestamp: new Date().toISOString(),
    ...
    checkWebAuthnSupport() {
        ...
        PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable()
            ...
            .catch(() => { // Dibiarkan jika ada error lain
                this.state.webauthnSupported = false;
                this.elements.webauthnUnsupported.style.display = 'block';
            });
    }
    ...
    async handleLogin() {
        ...
        try {
            ...
            if (responseData.authenticated) {
                ...
                this.elements.errorMsg.textContent = ''; // Pastikan pesan error hilang
                window.location.href = 'dashboard'; // Konsistenkan
            } else {
                this.handleFailedLogin(responseData.message || 'Invalid username or password.');
            }
        } catch (error) {
            console.error('Login error (network or worker response parsing issue):', error); // Dibiarkan untuk debugging
...
class SecuritySystem {
    ...
    loadAttemptData() {
        try {
            ...
            console.error('Failed to parse cookie data:', e); // Dibiarkan untuk debugging
    ...
    resetAttempts() { this.attemptData = this.getDefaultData(); this.saveAttemptData(); }
    isBlocked() {
        if (!this.attemptData.blockUntil) return false;
        const blockUntil = new Date(this.attemptData.blockUntil);
        const now = new Date();
        if (now > blockUntil) { this.resetAttempts(); return false; }
        return true;
    }
